...
export default function cellphone(value: unknown, options: Options = { 
  ...
  if (options?.locale && options.locale in regexes) {
    return optionalValidations(value, options) && regexes[options.locale].test(value);
  } 
  ...
  if (options?.validateAreaCodes) {
    switch (options.locale) {
      case 'pt-BR':
        ...
        const knownInvalidAreaCodes = [10, 20, 23, 25, 26, 29, 30, 36, 39, 40, 50, 52, 56, 57, 58, 59, 60, 70, 72, 76, 78, 80, 90    ]
        if (areaCode[0] === '0' || areaCode[1] === '0' || knownInvalidAreaCodes.includes(Number(areaCode))) return false
        ...
        if (first !== MOBILE_VALID_FIRST_DIGIT || second === MOBILE_INVALID_SECOND_DIGIT) return false;
    }
  }
...
const regexes: Record<string, RegExp> = {
  ...
  'fr-CA': /^((\+1|1)?( |-)?)?(\([2-9][0-9]{2}\)|[2-9][0-9]{2})( |-)?([2-9][0-9]{2}( |-)?[0-9]{4})$/,
  'fr-BE': /^(\+?32|0)4\d{8}$/,
